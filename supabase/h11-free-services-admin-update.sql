-- =====================================================
-- GreenroomID H11 - Free Services Admin + Usage Counter
-- Fungsi ini hanya menyimpan statistik event penggunaan.
-- File/gambar client dari Image to Table TIDAK disimpan ke database atau storage.
-- =====================================================

-- 1. Label statistik baru di landing.
INSERT INTO public.landing_content (content_key, content_value, label, group_name, sort_order)
VALUES
  ('stats_free_services_usage', 'Penggunaan Gratis', 'Label Penggunaan Layanan Gratis', 'Ringkasan Statistik', 12)
...
-- 3. Log angka penggunaan. Tidak ada payload file, nama file asli, gambar, atau hasil PDF.
CREATE TABLE IF NOT EXISTS public.free_service_usage_events (
  ...
  created_at timestamptz DEFAULT now(),
  CONSTRAINT free_service_usage_action_check CHECK (action IN ('use', 'download_pdf', 'print'))
);
...
-- 4. RLS tetap aktif. Akses browser memakai RPC security definer di bawah.
...
BEGIN
  ...
  v_action := CASE
    WHEN p_action IN ('use', 'download_pdf', 'print') THEN p_action
    ELSE 'use'
  END;
...
AS $$
  SELECT
    ...
    count(e.id) FILTER (WHERE e.action = 'download_pdf')::bigint AS download_pdf_count,
